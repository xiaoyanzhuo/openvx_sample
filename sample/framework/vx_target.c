...
VX_API_ENTRY vx_target VX_API_CALL vxGetTargetByIndex(vx_context context, vx_uint32 index)
{
    ...
    if (ownIsValidContext(context) == vx_true_e)
    {
        if (index < context->num_targets)
        {
            ...
            ownIncrementReference(&target->base, VX_EXTERNAL);
            ownPrintTarget(target, index);
        }
        else
        {
            VX_PRINT(VX_ZONE_ERROR, "Incorrect index value\n");
            vxAddLogEntry(&context->base, VX_ERROR_INVALID_PARAMETERS, "Incorrect index value\n");
            target = (vx_target_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
        }
    }
    else
    {
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseTarget(vx_target *t)
{
    ...
    if (ownIsValidSpecificReference((vx_reference_t *)target, VX_TYPE_TARGET) == vx_true_e)
    {
        ownDecrementReference(&target->base, VX_EXTERNAL);
    } else {
...
VX_API_ENTRY vx_status VX_API_CALL vxAssignNodeAffinity(vx_node node, vx_target target)
{
    ...
    if (ownIsValidSpecificReference(&node->base, VX_TYPE_NODE) == vx_true_e &&
        ownIsValidSpecificReference(&target->base, VX_TYPE_TARGET) == vx_true_e)
    {
