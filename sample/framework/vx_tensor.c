...
void ownInitTensor(vx_tensor tensor, const vx_size* dimensions, vx_size number_of_dimensions, vx_enum data_type, vx_int8 fixed_point_position)
{
    ...
    tensor->number_of_dimensions = number_of_dimensions;
    ...
    for (vx_uint32 i = number_of_dimensions; i < VX_MAX_TENSOR_DIMENSIONS; i++)
    {
        ...
        if (ownAllocateTensorMemory(tensor) != NULL)
        {
                ...
                (tensor->data_type == VX_TYPE_UINT8 && image_format == VX_DF_IMAGE_U8))
            {
                if (tensor->number_of_dimensions == 3)
                {
                    ...
                    {
                        vx_uint32 image_byte_size = tensor->dimensions[1] * tensor->dimensions[0] * ownSizeOfType(tensor->data_type);
                        ...
                        addr.stride_x = ownSizeOfType(tensor->data_type);
                        addr.stride_y = tensor->dimensions[0] * ownSizeOfType(tensor->data_type);
        ...
        vx_int8 fixed_point_position)
{
    ...
    if (ownIsValidSpecificReference(gref, VX_TYPE_GRAPH) == vx_true_e)
    {
        ...
        else
        {
            ...
            tensor = (vx_tensor)ownGetErrorObject(tensor->base.context, VX_ERROR_INVALID_FORMAT);
        }
    }
