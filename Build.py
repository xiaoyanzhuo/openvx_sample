...
def main():
    ...
    parser.add_option("--openmp", dest="openmp", help="Add -DEXPERIMENTAL_USE_OPENMP=ON to build also OpenMP target [Default False]", default=False, action='store_true')
...
#        cmd += ['-DOPENVX_USE_NN_16=ON']
    ...
    if options.openmp:
        cmd += ['-DEXPERIMENTAL_USE_OPENMP=ON']
    if options.f16:
